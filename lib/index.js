...
export function apply(ctx, config = {}) {
  ...
  async function fetchPricing() {
    if (pricingCache && Date.now() - pricingCache.at < pricingCacheMs) {
      // 命中缓存：价格数据不变，但当前时段必须实时计算（时段会随时间跨越）
      const p = pricingCache.payload
      return { ...p, currentPeriod: currentPeriod(p.peakSlots, p.weekendFlat, p.weekendFlatStart) }
    }
