...
function parsePricingHtml(html) {
  ...
  // 7) 高峰时段：从页面备注提取“高峰时段为北京时间 9:00 - 12:00、14:00 - 18:00”
  let peakSlots = FALLBACK_PEAK_SLOTS
  const noteMatch = html.match(/高峰时段为北京时间\s*([\d:]+)\s*-\s*([\d:]+)[、，,]\s*([\d:]+)\s*-\s*([\d:]+)/)
  if (noteMatch) {
    const slots = []
 ...
 */
function currentPeriod(peakSlots) {
  const now = new Date()
  ...
  async function fetchPricing() {
    ...
    pricingInflight ??= (async () => {
      try {
        ...
        const { models, peakSlots } = parsePricingHtml(html)
        const payload = {
          ok: true,
          models,
          peakSlots,
          currentPeriod: currentPeriod(peakSlots),
          fetchedAt: Date.now(),
          source: pricingUrl,
        }
